...
function shuffle (src) {
  ...
  function start (){
    if (gameStatus.active === false) {
       ...
       gameStatus.scrambled = shuffle(gameStatus.word)
       console.log(` guess the name of this country: ${gameStatus.scrambled}`)
      ...
      guessed = guessed.toLowerCase()
       if(guessed == gameStatus.word){
          ...
          if(gameStatus.words.length > 0){
            gameStatus.word = gameStatus.words.shift()
            gameStatus.scrambled = shuffle(gameStatus.word)
            console.log(`The next scrambled word is: ${gameStatus.scrambled}`)
          }else{
            ...
            if (gameStatus.strikes < gameStatus.maxStrikes ){
              ...
              console.log(`You're wrong,try again this county name: ${gameStatus.scrambled} ${gameStatus.strikes}`)
              
            }else{
              console.log(`maximum number of strikes reached. Your score is: ${gameStatus.poits}.`)
              gameStatus.active = false
            }
          }
          } else{
